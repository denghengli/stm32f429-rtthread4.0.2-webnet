...
void LTDC_Init(LTDC_InitExTypeDef* LTDC_InitStruct)
{
...
void LTDC_StructInit(LTDC_InitExTypeDef* LTDC_InitStruct)
{
...
/* Check the parameters */
  assert_param(IS_LTDC_PIXEL_FORMAT(LTDC_Layer_InitStruct->LTDC_PIXEL_FORMAT));
...
void LTDC_LayerStructInit(LTDC_Layer_InitTypeDef * LTDC_Layer_InitStruct)
{
  ...
  LTDC_Layer_InitStruct->LTDC_PixelFormat = LTDC_PIXEL_FORMAT_ARGB8888;
...
void LTDC_LayerPosition(LTDC_Layer_TypeDef* LTDC_Layerx, uint16_t OffsetX, uint16_t OffsetY)
{
  ...
  if (tempreg == LTDC_PIXEL_FORMAT_ARGB8888)
  {
    temp = 4;
  }
  else if (tempreg == LTDC_PIXEL_FORMAT_RGB888)
  {
    temp = 3;
  }
  else if ((tempreg == LTDC_PIXEL_FORMAT_ARGB4444) || 
          (tempreg == LTDC_PIXEL_FORMAT_RGB565)    ||  
          (tempreg == LTDC_PIXEL_FORMAT_ARGB1555)  ||
          (tempreg == LTDC_PIXEL_FORMAT_AL88))
  {
    temp = 2;  
  }
  else
...
/**
  ...
  *         one of the following values:@ref LTDC_PIXEL_FORMAT.   
  * @param  LTDC_layerx: Select the layer to be configured, this parameter can be 
  *         one of the following values: LTDC_Layer1, LTDC_Layer2   
  * @retval Reload of the shadow registers values must be applied after layer 
