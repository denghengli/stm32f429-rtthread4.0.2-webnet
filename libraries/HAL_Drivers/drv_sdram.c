...
static int SDRAM_Init(void)
{
    ...
    else
    {
        ...
        LOG_D("sdram init success, mapped at 0x%X, size is %d bytes, data width is %d", SDRAM_BANK_ADDR, MEMHEAP_SDRAM_SIZE, SDRAM_DATA_WIDTH);
#ifdef RT_USING_MEMHEAP_AS_HEAP
        /* If RT_USING_MEMHEAP_AS_HEAP is enabled, SDRAM is initialized to the heap */
        rt_memheap_init(&system_heap, "sdram", (void *)SDRAM_BANK_ADDR, MEMHEAP_SDRAM_SIZE);
#endif
    }
...
int sdram_test(void)
{
    ...
    /* write data */
    LOG_D("Writing the 1024 bytes data, waiting....");
    start_time = rt_tick_get();
    for (i = 0; i < 1024 / data_width; i++)
    {
#if SDRAM_DATA_WIDTH == 8
