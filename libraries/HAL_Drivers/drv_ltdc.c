...
ltdc_dev lcdltdc;			//管理LCD LTDC的重要参数
...
{   
	...
	lcdltdc.vfp = 15;				    //垂直前廊
	LTDC_Clk_Set(300,4,RCC_PLLSAIDIVR_4); //设置像素时钟396 3 4 --> 33M(如果开双显,需要降低DCLK到:18.75Mhz  300/4/4,才会比较好)
	...
	LTDC_Handler.Init.AccumulatedHBP = lcdltdc.hsw + lcdltdc.hbp - 1; //水平同步后沿宽度		46
	LTDC_Handler.Init.AccumulatedVBP = lcdltdc.vsw + lcdltdc.vbp - 1; //垂直同步后沿高度		15
	//    LTDC_Handler.Init.AccumulatedActiveW = lcdltdc.hsw+lcdltdc.hbp + lcdltdc.pwidth - 1;//有效宽度		846
	//    LTDC_Handler.Init.AccumulatedActiveH = lcdltdc.vsw+lcdltdc.vbp + lcdltdc.pheight - 1;//有效高度		495
	//    LTDC_Handler.Init.TotalWidth = lcdltdc.hsw + lcdltdc.hbp + lcdltdc.pwidth + lcdltdc.hfp - 1;   //总宽度	1056
	//   LTDC_Handler.Init.TotalHeigh = lcdltdc.vsw + lcdltdc.vbp + lcdltdc.pheight + lcdltdc.vfp - 1;  //总高度	511
...
void HAL_LTDC_MspInit(LTDC_HandleTypeDef* hltdc)
{
    ...
    //初始化PB6，背光引脚
    GPIO_Initure.Pin = GPIO_PIN_6;                //PB6推挽输出，控制背光
